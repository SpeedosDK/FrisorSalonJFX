...
public class BestillingRepo  implements IBestillingRepository{
    ...
    public void createBestilling(Bestilling bestilling) {
        String sql = "INSERT INTO bestillinger (bestilling_dato, bestilling_tid, kunde_id, klippeType_id, medarbejder_id, aktiv) VALUES (?, ?, ?, ?, ?, ?)";
...
//            preparedStatement.setInt(1, bestilling.getId());
            ...
            preparedStatement.setBoolean(6, bestilling.getAktiv());
    ...
    public ObservableList<Bestilling> readBestillinger() {
        ...
        String sql = "SELECT b.bestilling_id, b.bestilling_dato, b.bestilling_tid, b.aktiv, " +
        ...
        try (Connection connection = DatabaseConnection.getConnection();
            ...
            while (resultSet.next()) {
                ...
                bestillinger.add(new Bestilling(id, medarbejder, dato, tid, kunde, klippetype, aktiv));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public void updateBestilling(Bestilling bestilling) {
        String sql = "UPDATE bestillinger SET bestilling_dato = ?, bestilling_tid = ?, kunde_id = ?, klippeType_id = ?, medarbejder_id = ?, aktiv = ? WHERE bestilling_id = ?";
        try (Connection connection = DatabaseConnection.getConnection();
            ...
            preparedStatement.setInt(5, bestilling.getMedarbejder().getId());
            preparedStatement.setBoolean(6, bestilling.getAktiv());
            preparedStatement.setInt(7, bestilling.getId());
            ...
            if (rowsAffected > 0) {
                ...
                System.out.println("Ingen bestilling fundet med det ID.");
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
        ...
        try (Connection connection = DatabaseConnection.getConnection();
            ...
            try (ResultSet resultSet = preparedStatement.executeQuery()) {
                while (resultSet.next()) {
                    ...
                    foundBestillinger.add(new Bestilling(id, medarbejder, dato, tid, kunde, newKlippetype, aktiv));
                }
            }
        } catch (SQLException e) {
            e.printStackTrace();
